...
export default {
  ...
  computed: {
    ...
    messageForShow() {
        let item = this.conversation.lastMessage

        if (item.isRevoked) {
            if (item.fromAccount === this.currentUserProfile.userID) {
                return '你撤回了一条消息'
            }
            if (this.conversation.type === this.TIM.TYPES.CONV_C2C) {
                return '对方撤回了一条消息'
            }
            return `${item.fromAccount}撤回了一条消息`
        }

        let text = item.messageForShow
        if(item.messageForShow == "[自定义消息]"){
            let data = JSON.parse(item.payload.data)
            if(data){
                if(!this.bpw.null(data.retailPrice)){
                    text = "[商品信息]"
                }
            }
        }
